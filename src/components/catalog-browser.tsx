  ...
  return (
    <div className={mode === 'shop' ? 'catalog-layout' : 'category-shop'}>
      ...
      <section className={mode === 'shop' ? 'catalog-results' : 'category-results'} aria-live="polite">
        ...
        {visibleProducts.length ? <div className={`catalog-products ${view === 'list' ? 'list-view' : ''}`}>{visibleProducts.map((product, index) => <ProductCard key={product.slug} p={product} index={index} />)}</div> : <div className="catalog-empty"><Search aria-hidden="true" /><h2>No templates match these filters</h2><p>Try raising the price limit or clearing one of the selected filters.</p><button type="button" onClick={clearFilters}>Clear filters</button></div>}
      </section>
    </div>
  );
}
