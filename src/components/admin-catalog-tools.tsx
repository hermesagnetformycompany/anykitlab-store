...
async function api<T>(method: string, body?: unknown): Promise<T> {
  ...
  if (!response.ok) {
    const error = new Error(payload.error || 'Catalog action failed.') as Error & {requiresCascade?: boolean};
...
export function AdminCatalogTools() {
  ...
  async function assignMedia() {
    setBusy(true);
    try {
      await api('POST', {action: 'assign-media', productSlug, mediaIds: selectedMedia, coverId: effectiveCoverId});
      setSelectedMedia([]);
      setCoverId('');
  ...
  return (
    <>
      <button className="catalog-tools-launcher" type="button" onClick={openTools}><LayoutTemplate aria-hidden="true" />Catalog tools</button>
      {open && (
        <div className="catalog-tools-backdrop" role="presentation">
          <section className="catalog-tools-panel" role="dialog" aria-modal="true" aria-labelledby="catalog-tools-title">
            <header>
              <div><span>LIVE DATA CONTROL</span><h2 id="catalog-tools-title">Catalog tools</h2><p>Publish templates, manage categories and collections, and assign real media.</p></div>
            ...
            <nav aria-label="Catalog tools sections">
              ...
              <button type="button" onClick={() => void load()} title="Refresh"><RefreshCw aria-hidden="true" /></button>
            </nav>
            ...
            <div className="catalog-tools-body">
              {loading ? <p>Loading live catalog…</p> : tab === 'Templates' ? (
                <TemplateTools products={data.products} busy={busy} onUpdate={update} onDelete={remove} />
              ) : tab === 'Categories' ? (
                <CategoryTools rows={data.categories} editing={editing} setEditing={setEditing} busy={busy} onUpdate={update} onDelete={remove} />
              ) : tab === 'Collections' ? (
                <CollectionTools rows={data.collections} editing={editing} setEditing={setEditing} busy={busy} onUpdate={update} onDelete={remove} />
              ) : (
                <MediaTools media={assignableMedia} products={data.products} selected={selectedMedia} setSelected={setSelectedMedia} coverId={effectiveCoverId} setCoverId={setCoverId} productSlug={productSlug} setProductSlug={setProductSlug} busy={busy} onAssign={assignMedia} onDelete={remove} />
              )}
            </div>
          </section>
        </div>
...
function CategoryTools({rows, editing, setEditing, busy, onUpdate, onDelete}: {rows: CategoryRow[]; editing: EditTarget; setEditing: (value: EditTarget) => void; busy: boolean; onUpdate: (kind: 'category', id: string, values: Record<string, unknown>) => Promise<void>; onDelete: (kind: 'category', id: string, label: string) => Promise<void>}) {
...
function CollectionTools({rows, editing, setEditing, busy, onUpdate, onDelete}: {rows: CollectionRow[]; editing: EditTarget; setEditing: (value: EditTarget) => void; busy: boolean; onUpdate: (kind: 'collection', id: string, values: Record<string, unknown>) => Promise<void>; onDelete: (kind: 'collection', id: string, label: string) => Promise<void>}) {
